...
import { getWorkouts, getWorkoutDetails } from '../utils/storage';
...
const Records = () => {
  ...
  useEffect(() => {
    loadWorkouts();
    const handleWorkoutUpdate = () => loadWorkouts();
  ...
  const loadWorkouts = async () => {
    ...
    const detailed = await Promise.all(list.map((w) => getWorkoutDetails(w.id)));
